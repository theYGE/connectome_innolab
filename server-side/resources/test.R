...
image_file ="./output_data/filtered_func_data_clean.nii"
...
# Current Atlas is 2mm, 7-Networks, 400 parcels 
Atlas_file ="Schaefer2018_400Parcels_7Networks_order_FSLMNI152_2mm.nii.gz"
...
Fisher_z_transform = T
### Atlas preprocessing ###
# Mask_atlas_with_GM = boolean (T or F)
# => if T => GM_mask = pxath to GM mask nifti (string)
Mask_atlas_with_GM = T
GM_mask="MNI152_T1_2mm_brain_mask_bin.nii.gz"
### fMRI preprocessing ###
## filtering
...
library("RNifti")
library("tictoc")
FC_computation_atlas_based <- function(image_file, output_path, Atlas_file, Fisher_z_transform, Mask_atlas_with_GM, GM_mask, detrend, bandpass, TR, motion_regression, motion_file, WM_nuisance, WM_mask, CSF_nuisance,CSF_mask){
  ...
  if(Fisher_z_transform==T){
    ...
    # return(zFC_matrix)
    write.csv(zFC_matrix, file = output_path, row.names = FALSE)
  }
  
  if(Fisher_z_transform==F){
    # return(FC_matrix)
    write.csv(FC_matrix, file = output_path, row.names = FALSE)
  }
}
...
# input_image_file = paste(c(workPath,file,"/",image_file),collapse="")
input_image_file = "./output_data/filtered_func_data_clean.nii"
print(input_image_file)
# output_file=paste(c(workPath,file,"/",file,".csv"),collapse="")
output_file = "./output_data/output.csv"
if(file.exists(path=input_image_file)){
  print(input_image_file)
  print(output_file)
  FC_computation_atlas_based(image_file=input_image_file, output_path=output_file,Atlas_file, Fisher_z_transform, Mask_atlas_with_GM, GM_mask, detrend, bandpass, TR, motion_regression, motion_file, WM_nuisance, WM_mask, CSF_nuisance,CSF_mask)
}
